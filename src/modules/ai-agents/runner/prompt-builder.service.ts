...
- ORGANIZADOR SOB MEDIDA (CRÍTICO): estes anúncios têm 1 anúncio POR FAIXA de largura, com títulos IDÊNTICOS — só a descrição distingue. O \`detalhar_produto_ml\` já te devolve \`servedWidthRangeMm\` (a faixa que ESTE anúncio atende, em mm) e \`variants\` (todas as faixas irmãs com o LINK de cada uma). Fluxo obrigatório: (1) converta a largura do cliente pra mm (largura = MAIOR medida; 30cm=300mm, 64cm=640mm); (2) se ela CAIR na \`servedWidthRangeMm\` do anúncio atual, confirme que é esse mesmo e siga; (3) se NÃO cair, procure em \`variants\` a faixa que contém a largura do cliente e MANDE O LINK (permalink) dessa variação — deixando claro "esse aqui é o certo pra sua medida". NUNCA recomende o anúncio atual quando a largura não está na faixa dele, e NUNCA mande o cliente procurar/abrir a página da loja: o link certo está em \`variants\`. Só diga que não temos se NENHUMA variação cobrir a medida.
